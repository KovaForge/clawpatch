...
const minimaxProvider: Provider = {
  ...
  async check(): Promise<string> {
    ...
    if (!key) {
      throw new ClawpatchError("MINIMAX_API_KEY environment variable is not set", 4, "provider-auth");
    }
    return `minimax (key prefix: ${key.slice(0, 8)}…)`;
  },
  async map(_root: string, prompt: string, options: ProviderOptions): Promise<AgentMapOutput> {
    const output = await runMinimaxJson(prompt, options, agentMapJsonSchema, true);
  ...
  async review(
    ...
    const output = await runMinimaxJson(prompt, options, reviewJsonSchema, true);
    return parseReviewOutput(output);
  },
  async fix(_root: string, prompt: string, options: ProviderOptions): Promise<FixPlanOutput> {
    const output = await runMinimaxJson(prompt, options, fixPlanJsonSchema, false);
    return parseOrThrow(fixPlanOutputSchema, output, "minimax fix-plan");
  },
  async revalidate(
    _root: string,
    prompt: string,
    ...
    const output = await runMinimaxJson(prompt, options, revalidateJsonSchema, true);
...
async function runMinimaxJson(
  ...
  if (!apiKey) {
    throw new ClawpatchError("MINIMAX_API_KEY is not set", 4, "provider-auth");
  }
  ...
  if (parsed === null) {
    const preview = safeProviderPreview(content);
    throw new ClawpatchError(
      `minimax produced unparseable JSON (preview: ${preview})`,
      8,
      "malformed-output",
    );
  }
